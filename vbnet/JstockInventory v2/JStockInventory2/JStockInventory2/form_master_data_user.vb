...
Imports System.Text
Imports System.Math
Public Class form_master_data_user
    ...
    Public Sub prepare_data_pelengkap()
        Dim methodName As String = System.Reflection.MethodBase.GetCurrentMethod().Name
        Try
            Me.total_data = data_obj.get_total_rows("master_user")
    ...
    Private Sub form_master_data_user_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Dim methodName As String = System.Reflection.MethodBase.GetCurrentMethod().Name
        Try
            basic_op.hide_id(label_id, text_id)
            form_master_data_user_add.added_data = 0
            Me.saved_data = 0
            Me.page = 1
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        form_master_data_user_add.Show()
        basic_op.setfokus(form_master_data_user_add)
    End Sub

    Private Sub Timer1_Tick(sender As Object, e As EventArgs) Handles Timer1.Tick
        Dim methodName As String = System.Reflection.MethodBase.GetCurrentMethod().Name
        Try
            If form_master_data_user_add.added_data = 1 Or Me.saved_data = 1 Then
                form_master_data_user_add.added_data = 0
                Me.saved_data = 0
                Call prepare_data_pelengkap()
                Call LoadData(page)
    ...
    Public Sub LoadData(ByVal hal As Integer)
        Try
            ...
            sql = "SELECT id,user, level, nama_lengkap,hp from master_user  order by `id` desc limit " + pg_str + ",10"
    ...
    Public Sub selected_view(e As DataGridViewCellEventArgs)
        ...
        Try
            If (e.RowIndex >= 0) And DGView.CurrentCell.Value <> Nothing Then
                ...
                text_user.Text = row.Cells("user").Value.ToString
                If row.Cells("level").Value.ToString = "admin" Then
                    Me.combo_level.SelectedIndex = Me.combo_level.FindString("admin").ToString
                Else
                    Me.combo_level.SelectedIndex = Me.combo_level.FindString("operator").ToString
                End If
                text_nama_lengkap.Text = row.Cells("nama_lengkap").Value.ToString
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        ...
        Dim methodName As String = System.Reflection.MethodBase.GetCurrentMethod().Name
        Try
            If Len(text_user.Text) < 1 Then
                MsgBox("username harus diisi")
            Else
                Dim ret = 0
                Dim user = text_user.Text
                Dim level = combo_level.Text
                Dim nama_lengkap = text_nama_lengkap.Text
                Dim hp = text_hp.Text
                Dim id = text_id.Text
                Dim sql = "update `master_user` set `user` = '" + user + "', `level` = '" + level + "', `nama_lengkap` = '" + nama_lengkap + "', `hp` = '" + hp + "' where `id` like '" + id + "'"
    ...
    Private Sub Button5_Click(sender As Object, e As EventArgs) Handles Button5.Click
        Dim methodName As String = System.Reflection.MethodBase.GetCurrentMethod().Name
        Try
            Dim hapus = basic_op.konfirmasi_hapus("Apakah data user akan dihapus ?", "Hapus Data User")
            If hapus = 1 Then
                Dim id = text_id.Text
                Dim res = 0
                res = data_obj.mysql_del("master_user", id)
                If res = 0 Then
