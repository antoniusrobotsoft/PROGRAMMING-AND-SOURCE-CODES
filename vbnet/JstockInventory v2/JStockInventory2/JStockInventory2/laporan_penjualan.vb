...
Public Class laporan_penjualan
    ...
    Private Sub tombol_simpan_Click(sender As Object, e As EventArgs) Handles tombol_simpan.Click
        ...
        Try
            ...
            ElseIf text_tanggal.Text.Contains("/") = False Then
                MsgBox("tanggal harus diisi dengan benar, format: dd/mm/yyyy, tanggal harus menggunakan pemisah /")
            ...
            Else
                ...
                If Val(tanggal) < 1 Or Val(tanggal) > 31 Then
                    MsgBox("hari tanggal harus diisi dengan benar, format: dd/mm/yyyy")
    ...
    Public Sub LoadData_sorting(ByVal hal As Integer, ByVal bulan As String, ByVal tahun As String)
        ...
        Try
            ...
            sql = "SELECT id, kode, nama_produk, harga, jumlah, waktu, tanggal, bulan, tahun FROM op_stok_keluar  where `bulan` = '" + bulan + "' and `tahun` = '" + tahun + "' and `mark` like 'penjualan'  order by `id` desc limit " + pg_str + ",10"
            Me.last_sql_for_csv = "select  full_date,kode,nama_produk, harga, jumlah, waktu FROM op_stok_keluar  where `bulan` = '" + bulan + "' and `tahun` = '" + tahun + "' and `mark` like 'penjualan'  order by `id` desc limit 0,500"
    ...
    Public Sub prepare_data_pelengkap()
        ...
        Try
            ...
            If current_mode_view = "default" Then
                Me.total_data = data_obj.get_total_rows_on_condition("op_stok_keluar", "mark", "penjualan")
            ElseIf current_mode_view = "range" Then
                ...
                Me.total_data = data_obj.get_total_rows_between_with_condition("op_stok_keluar", "mark", "penjualan", "full_Date", full_date_dari, full_date_hingga)
            Else
                If (bulan <> "Pilih Bulan") And (tahun <> "Pilih Tahun") Then
                    Me.total_data = data_obj.get_total_rows_on_3condition("op_stok_keluar", "bulan", bulan, "tahun", tahun, "mark", "penjualan")
                Else
    ...
    Private Sub tombol_csv_Click(sender As Object, e As EventArgs) Handles tombol_csv.Click
        ...
        form_csv.additional_mark = "penjualan"
    ...
    Public Sub selected_view(e As DataGridViewCellEventArgs)
        ...
        Try
            If (e.RowIndex >= 0) And DGView.CurrentCell.Value <> Nothing Then
                ...
                text_tanggal.Text = row.Cells("tanggal").Value.ToString
    ...
    Public Sub LoadData(ByVal hal As Integer)
        ...
        Try
            ...
            If Me.current_mode_view = "range" Then
                Dim full_date_dari = basic_op.return_mysql_date_format(DateTimePicker_dari.Text)
                Dim full_date_hingga = basic_op.return_mysql_date_format(DateTimePicker_hingga.Text)
                sql = "SELECT id, kode, nama_produk, harga, jumlah, waktu, tanggal, bulan, tahun FROM op_stok_keluar  where (`mark` = 'penjualan') and (full_date BETWEEN '" + full_date_dari + "' AND '" + full_date_hingga + "') order by `id` desc limit " + pg_str + ",10"
                Me.last_sql_for_csv = "select  full_date,kode,nama_produk, harga, jumlah, waktu FROM op_stok_keluar  where (`mark` = 'penjualan') and (full_date BETWEEN '" + full_date_dari + "' AND '" + full_date_hingga + "') order by `id` desc limit 0,500"
                Me.sql_for_print = Me.last_sql_for_csv
            Else
                sql = "SELECT id, kode, nama_produk, harga, jumlah, waktu, tanggal, bulan, tahun FROM op_stok_keluar  where `mark` = 'penjualan' order by `id` desc limit " + pg_str + ",10"
                Me.last_sql_for_csv = "select  full_date,kode,nama_produk, harga, jumlah, waktu FROM op_stok_keluar  where (`mark` = 'penjualan') order by `id` desc limit 0,500"
                Me.sql_for_print = Me.last_sql_for_csv
            End If
    ...
    Private Sub tombol_print_preview_Click(sender As Object, e As EventArgs) Handles tombol_print_preview.Click
        ...
        Try
            ...
            End If
            form_print_prepare.current_file = "laporan_penjualan_preview.html"
            print_generator.create_dokumen_print_laporan_pembelian_or_penjualan(Me.sql_for_print, jumlah_maks_data, "preview", "penjualan")
    ...
    Private Sub tombol_print_Click(sender As Object, e As EventArgs) Handles tombol_print.Click
        ...
        Try
            ...
            If jumlah_maks_data < 1 Then
                jumlah_maks_data = 300
            End If
            form_print.current_file = "laporan_penjualan_print.html"
            print_generator.create_dokumen_print_laporan_pembelian_or_penjualan(Me.sql_for_print, jumlah_maks_data, "print", "penjualan")
            form_print.Show()
    ...
    Private Sub tombol_delete_all_Click(sender As Object, e As EventArgs) Handles tombol_delete_all.Click
        Dim methodName As String = System.Reflection.MethodBase.GetCurrentMethod().Name
        Try
            Dim hapus = basic_op.konfirmasi_hapus("Apakah Anda yakin untuk menghapus seluruh data penjualan ?", "Hapus Data Seluruh Penjualan")
            If hapus = 1 Then
                form_delete_all.table_name = "op_stok_keluar"
                form_delete_all.additional_condition = "penjualan"
