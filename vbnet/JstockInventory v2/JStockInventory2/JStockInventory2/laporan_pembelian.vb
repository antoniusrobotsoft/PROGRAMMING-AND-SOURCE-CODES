...
Public Class laporan_pembelian
    ...
    Public Sub LoadData_sorting(ByVal hal As Integer, ByVal bulan As String, ByVal tahun As String)
        ...
        Try
            ...
            sql = "SELECT id, kode, nama_produk, harga, jumlah, waktu, tanggal, bulan, tahun FROM op_stok_masuk  where `bulan` = '" + bulan + "' and `tahun` = '" + tahun + "' and `mark` like 'pembelian'  order by `id` desc limit " + pg_str + ",10"
            Me.last_sql_for_csv = "select full_date,kode,nama_produk, harga, jumlah, waktu from op_stok_masuk where (`mark` = 'pembelian') and (`bulan` = '" + bulan + "' and `tahun` = '" + tahun + "')  order by `id` desc limit 0,500"
    ...
    Private Sub tombol_csv_Click(sender As Object, e As EventArgs) Handles tombol_csv.Click
        ...
        form_csv.additional_mark = "pembelian"
    ...
    Public Sub LoadData(ByVal hal As Integer)
        ...
        Try
            ...
            If Me.current_mode_view = "range" Then
                ...
                sql = "SELECT id, kode, nama_produk, harga, jumlah, waktu, tanggal, bulan,tahun FROM op_stok_masuk where (`mark` = 'pembelian') and (full_date BETWEEN '" + full_date_dari + "' AND '" + full_date_hingga + "')  order by `id` desc limit " + pg_str + ",10"
                Me.last_sql_for_csv = "select full_date,kode,nama_produk, harga, jumlah, waktu from op_stok_masuk where (`mark` = 'pembelian') and (full_date BETWEEN '" + full_date_dari + "' AND '" + full_date_hingga + "')  order by `id` desc"
                Me.sql_for_print = Me.last_sql_for_csv
            Else
                sql = "Select id, kode, nama_produk, harga, jumlah, waktu, tanggal, bulan,tahun FROM op_stok_masuk where `mark` = 'pembelian'  order by `id` desc limit " + pg_str + ",10"
                Me.last_sql_for_csv = "select full_date,kode,nama_produk, harga, jumlah, waktu from op_stok_masuk where (`mark` = 'pembelian') order by `id` desc limit 0, 300"
                Me.sql_for_print = Me.last_sql_for_csv
            End If
    ...
    Public Sub prepare_data_pelengkap()
        ...
        Try
            ...
            End If
            If current_mode_view = "default" Then
                Me.total_data = data_obj.get_total_rows_on_condition("op_stok_masuk", "mark", "pembelian")
            ElseIf current_mode_view = "range" Then
                Dim full_date_dari = basic_op.return_mysql_date_format(DateTimePicker_dari.Text)
                Dim full_date_hingga = basic_op.return_mysql_date_format(DateTimePicker_hingga.Text)
                Me.total_data = data_obj.get_total_rows_between_with_condition("op_stok_masuk", "mark", "pembelian", "full_Date", full_date_dari, full_date_hingga)
            Else
                If (bulan <> "Pilih Bulan") And (tahun <> "Pilih Tahun") Then
                    Me.total_data = data_obj.get_total_rows_on_3condition("op_stok_masuk", "bulan", bulan, "tahun", tahun, "mark", "pembelian")
                Else
    ...
    Private Sub tombol_print_preview_Click(sender As Object, e As EventArgs) Handles tombol_print_preview.Click
        ...
        Try
            ...
            End If
            form_print_prepare.current_file = "laporan_pembelian_preview.html"
            print_generator.create_dokumen_print_laporan_pembelian_or_penjualan(Me.sql_for_print, jumlah_maks_data, "preview", "pembelian")
    ...
    Private Sub tombol_print_Click(sender As Object, e As EventArgs) Handles tombol_print.Click
        ...
        Try
            ...
            If jumlah_maks_data < 1 Then
                jumlah_maks_data = 300
            End If
            form_print.current_file = "laporan_pembelian_print.html"
            print_generator.create_dokumen_print_laporan_pembelian_or_penjualan(Me.sql_for_print, jumlah_maks_data, "print", "pembelian")
            form_print.Show()
    ...
    Private Sub tombol_delete_all_Click(sender As Object, e As EventArgs) Handles tombol_delete_all.Click
        Dim methodName As String = System.Reflection.MethodBase.GetCurrentMethod().Name
        Try
            Dim hapus = basic_op.konfirmasi_hapus("Apakah Anda yakin untuk menghapus seluruh data pembelian ?", "Hapus Data Seluruh Pembelian")
            If hapus = 1 Then
                form_delete_all.table_name = "op_stok_masuk"
                form_delete_all.additional_condition = "pembelian"
